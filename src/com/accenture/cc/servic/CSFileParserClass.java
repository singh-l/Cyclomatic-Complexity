...
public class CSFileParserClass
{
  ...
  int     index       = 0, multicommentCount = 0, ifCount = 0, ifElseCount = 0, doWhileCount = 0, whileCount = 0,
          ...
          conditionalORCount = 0, unvCount = 0, bitWiseAndCount = 0, bitWiseOrCount = 0, bitWiseXorCount = 0, forEachCount=0;
  ...
  public void collectTokens(String codeLine)
  {
    ...
    if (codeLine != null && !codeLine.equals(""))
    {
     // System.out.println(codeLine);
      ...
      else if (codeLine.matches("else\\s+") || codeLine.matches("\\s+else") || codeLine.matches("\\s+else\\s+"))
      {
        ifElseCount++;
        unvCount++;
        ifCount--;
      ...
      else if (codeLine.matches("\\s+do") || codeLine.matches("do\\s+") || codeLine.matches("\\s+do\\s+"))
      {
      ...
      if ((codeLine.matches(".*foreach\\s+\\(.*") || codeLine.matches(".*foreach\\(.*")) &&
              codeLine.matches(".*[^a-zA-Z0-9&_$%@#!^+-/*öüÄÖÜ]foreach[^a-zA-Z0-9&_$%@#!^+-/*öüÄÖÜ].*"))
          {
      ...
      else if (codeLine.matches("\\s+finally") || codeLine.matches("finally\\s+") || codeLine.matches("\\s+finally\\s+"))
      {
        finallyBlockcount++;
        unvCount++;
      }
